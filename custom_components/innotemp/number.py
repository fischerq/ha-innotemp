...
def _extract_numbers_from_room_component(
    ...
    if isinstance(component_data, dict):
        ...
        _LOGGER.debug(f"Number: Unexpected component_data type: {type(component_data)} for room {room_attributes.get('var')}")
        return

    for component_item_data in components_to_process:
        if not isinstance(component_item_data, dict):
            _LOGGER.debug(f"Number: Skipping non-dict item in component_data list for room {room_attributes.get('var')}: {component_item_data}")
            continue
        ...
        for actual_entry in entries:
            if not isinstance(actual_entry, dict):
                _LOGGER.debug(f"Number: Skipping non-dict entry in entries list for room {room_attributes.get('var')}: {actual_entry}")
                continue
            ...
            if param_id and unit and unit != "ONOFFAUTO": # and access in ["user", "admin"]:
                # Further check if unit is numeric, e.g. by trying to assign device_class or known units
                is_potential_number = False
                if unit in [UnitOfTemperature.CELSIUS, UnitOfTemperature.FAHRENHEIT, PERCENTAGE, "K", "s", "min", "h", "bar", "rpm"]: # Add more known numeric units
                ...
                # For now, if it has a unit and is not ONOFFAUTO, and is in an 'entry', assume it's a number.
                    ...
                    _LOGGER.debug(f"Number: Entry with unit '{unit}' for param_id '{param_id}' treated as potential number (further checks in class).")
                ...
                if is_potential_number:
                    ...
                    entities_list.append(
                        InnotempNumber(
                            coordinator, entry, room_attributes, component_attributes, param_id, actual_entry
                        )
                    )
                else:
                    _LOGGER.debug(f"Number: Entry unit '{unit}' for {param_id} not recognized as numeric or is ONOFFAUTO.")
            # else:
...
async def async_setup_entry(
    ...
    if config_data is None:
        _LOGGER.warning("Innotemp number setup: config_data is None, skipping entity creation.")
        async_add_entities([])
        return

    entities = []
    _LOGGER.debug("Innotemp number setup: Received config_data (first 500 chars): %s", str(config_data)[:500])

    if not isinstance(config_data, dict):
        _LOGGER.error(f"Number: Config_data is not a dictionary. Type: {type(config_data)}. Data: {config_data}")
        async_add_entities([])
        return
    ...
    for top_level_key, top_level_value in config_data.items():
        ...
        if not actual_room_list and isinstance(top_level_value, str):
            try:
                ...
                _LOGGER.debug(f"Number: Could not parse string value for key {top_level_key} as JSON list.")
        ...
        for room_data_dict in actual_room_list:
            if not isinstance(room_data_dict, dict):
                _LOGGER.warning(f"Number: Item in room list for key '{top_level_key}' is not a dict: {room_data_dict}")
                continue

            room_attributes = room_data_dict.get("@attributes", {})
            if not room_attributes.get("var"):
                _LOGGER.warning(f"Number: Room missing '@attributes.var': {room_attributes}. Skipping.")
                continue
    ...
    if not entities:
        _LOGGER.info("No Number entities found in Innotemp configuration with current logic.")
    else:
...
class InnotempNumber(InnotempCoordinatorEntity, NumberEntity):
    ...
    def __init__(
        ...
        if unit == UnitOfTemperature.CELSIUS or unit == "c" or unit == "°c":
            ...
            self._attr_device_class = None # No specific NumberDeviceClass for generic percentage
            self._attr_native_min_value = 0.0
            self._attr_native_max_value = 100.0
            self._attr_native_step = 1.0
        elif unit == "k": # Assuming Kelvin for temperature differences/spreads
            self._attr_device_class = None # No direct NumberDeviceClass for Kelvin spread
            self._attr_native_min_value = 0.0
    ...
    def native_value(self) -> float | None:
        ...
        if self.coordinator.data is None:
            _LOGGER.debug(f"InnotempNumber.native_value: Coordinator data is None for {self.entity_id} (param_id: {self._param_id}).")
            return None

        value = self.coordinator.data.get(self._param_id)
        if value is None:
            _LOGGER.debug(f"InnotempNumber.native_value: Param_id {self._param_id} not found in coordinator data for {self.entity_id}.")
            return None

        try:
            return float(value)
        except (ValueError, TypeError):
            _LOGGER.warning(f"Could not convert number value '{value}' to float for {self.entity_id} (param_id: {self._param_id}).")
            return None
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        try:
            ...
            if success:
                _LOGGER.info(f"Successfully sent command for {self.entity_id} to set value to {value}.")
                await self.coordinator.async_request_refresh()
            else:
                _LOGGER.error(f"Failed to send command for {self.entity_id} to set value to {value}.")
        except Exception as e:
            _LOGGER.error(f"Error sending command for {self.entity_id} to set value to {value}: {e}", exc_info=True)
