...
class InnotempApiClient:
    ...
    async def async_api_request(self, method: str, endpoint: str, data: Optional[Dict[str, Any]] = None, attempt: int = 1) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with self._session.request(method, url, data=data) as response:
                ...
                if 'application/json' in response.headers.get('Content-Type', ''):
                    ...
                    _LOGGER.debug(f"Non-JSON response from {endpoint}: {await response.text()}")
                    return {}
            ...
            if e.status in [401, 403] and attempt == 1:
                ...
                if await self.async_login():
                    return await self.async_api_request(method, endpoint, data=data, attempt=2)
                else:
            ...
            _LOGGER.error(f"An unexpected error occurred during API request to {endpoint}: {e}")
            return None
    ...
    async def async_get_config(self) -> Optional[Dict[str, Any]]:
        ...
        if self._config is None:
            config_data = {"un": self._username, "pw": self._password, "date_string": "0"}
            self._config = await self.async_api_request("POST", "roomconf.read.php", data=config_data)
            if self._config:
    ...
    async def async_send_command(self, room_id: int, param: str, val_new: Any, val_prev: Any) -> bool:
        ...
        result = await self.async_api_request("POST", "value.save.php", data=command_data)
    ...
    async def _get_signal_names(self) -> Optional[list[str]]:
        ...
        if self._signal_names is None:
            init_data = {"init": "1"}
            response = await self.async_api_request("POST", "live_signal.read.php", data=init_data)
    ...
    async def async_sse_connect(self, callback: Callable[[Dict[str, Any]], Awaitable[None]]):
        ...
        async def sse_listener():
            ...
            while True:
                ...
                if not signal_names:
                    _LOGGER.error("Could not get signal names for SSE stream. Retrying in 60 seconds.")
                                    ...
                                    if isinstance(data_list, list):
                                        ...
                                        for i, value in enumerate(data_list):
                                            if i < len(signal_names):
                                                ...
                                                _LOGGER.warning(f"Received data for index {i} but only have {len(signal_names)} signal names. Data: {data_list}")
                                                break # Stop processing this message if indices don't match

                                        _LOGGER.debug(f"SSE received data: {processed_data}")
                                        await callback(processed_data)
                                except json.JSONDecodeError as e:
                                    _LOGGER.error(f"Failed to decode JSON from SSE data: {e} - Data: {line.decode('utf-8')}")
                                except Exception as e:
                                    _LOGGER.error(f"Error processing SSE data: {e} - Data: {line.decode('utf-8')}")

                            elif line.startswith(b'event:'):
                                _LOGGER.debug(f"SSE event received: {line[6:].strip().decode('utf-8')}")
                            elif line == b'':
                                pass # Keep-alive or empty line
                            else:
                                _LOGGER.debug(f"Unknown SSE line: {line.decode('utf-8')}")

                except aiohttp.ClientConnectorError as e:
                    _LOGGER.error(f"SSE connection error: {e}. Reconnecting in 30 seconds.")
                    await asyncio.sleep(30)
                except aiohttp.ClientResponseError as e:
                    _LOGGER.error(f"SSE response error: {e}. Attempting re-login and reconnect in 30 seconds.")
                    await self.async_login() # Attempt re-login
                    await asyncio.sleep(30)
                except Exception as e:
                    _LOGGER.error(f"Unexpected error in SSE listener: {e}. Reconnecting in 30 seconds.")
                    await asyncio.sleep(30)
