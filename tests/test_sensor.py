...
def mock_coordinator_success(hass: HomeAssistant):
    ...
    coordinator.data = {
        "sensors": [
            ...
            {"param": "generic_sensor", "label": "Generic Value"}, # Sensor with no unit in label
        ],
        "temp_sensor": "22.5",
    ...
    config_entry_mock = MagicMock(spec=config_entries.ConfigEntry, entry_id="test_entry_id")
...
def mock_coordinator_failure(hass: HomeAssistant):
    ...
    coordinator.data = {
        "sensors": [
            ...
            {"param": "sensor_no_label_data", "label": "No Label Sensor"}, # Data might be missing
    ...
    config_entry_mock = MagicMock(spec=config_entries.ConfigEntry, entry_id="test_entry_id_failure")
    type(coordinator).config_entry = PropertyMock(return_value=config_entry_mock)
...
@pytest.mark.asyncio
async def test_sensor_setup_and_state_success(hass: HomeAssistant, mock_coordinator_success):
    """Test successful sensor setup, state, unit, and device class."""
    add_entities_callback = MagicMock(spec=AddEntitiesCallback)

    # Setup sensors
    await async_setup_entry(hass, mock_coordinator_success.config_entry, add_entities_callback)
    ...
    temp_sensor_entity = next(e for e in entities if e.entity_description.key == temp_sensor_config["param"])
    assert isinstance(temp_sensor_entity, InnotempSensor)
    assert temp_sensor_entity.name == "Living Room Temperature (°C)" # Full label as name
    assert temp_sensor_entity.unique_id == f"{mock_coordinator_success.config_entry.entry_id}-{temp_sensor_config['param']}"
    ...
    assert temp_sensor_entity.device_info["identifiers"] == {(DOMAIN, mock_coordinator_success.config_entry.entry_id)}


    # Sensor 2: Humidity
    humidity_sensor_config = mock_coordinator_success.data["sensors"][1]
    humidity_sensor_entity = next(e for e in entities if e.entity_description.key == humidity_sensor_config["param"])
    ...
    power_sensor_entity = next(e for e in entities if e.entity_description.key == power_sensor_config["param"])
    ...
    generic_sensor_entity = next(e for e in entities if e.entity_description.key == generic_sensor_config["param"])
...
async def test_sensor_state_failure(hass: HomeAssistant, mock_coordinator_failure):
    ...
    await async_setup_entry(hass, mock_coordinator_failure.config_entry, add_entities_callback)
    ...
    missing_data_sensor_entity = next(e for e in entities if e.entity_description.key == missing_data_sensor_config["param"])
    ...
    assert missing_data_sensor_entity.unit_of_measurement == "°C" # Unit should still be parsed
    ...
    no_label_data_sensor_entity = next(e for e in entities if e.entity_description.key == no_label_data_sensor_config["param"])
    assert no_label_data_sensor_entity.name == "No Label Sensor"
    assert no_label_data_sensor_entity.state is None # Assuming 'sensor_no_label_data' key is missing
    ...
    none_value_sensor_entity = next(e for e in entities if e.entity_description.key == none_value_sensor_config["param"])
