...
import pytest # Added
from homeassistant import config_entries, data_entry_flow, setup # data_entry_flow added
...
async def test_config_flow_success(hass: HomeAssistant):
    ...
    with patch(
        ...
        assert result["type"] == data_entry_flow.FlowResultType.FORM # Changed to use imported data_entry_flow
...
async def test_successful_flow_from_subtask(hass: HomeAssistant) -> None: # mock_setup_entry is auto-used from conftest.py
...
async def test_flow_invalid_host_is_http(hass: HomeAssistant) -> None:
    ...
    assert result2["type"] == data_entry_flow.RESULT_TYPE_FORM
    assert result2["errors"] is not None
    assert result2["errors"].get(CONF_HOST) == "Hostname cannot be 'http' or 'https'. Enter a valid IP address or hostname."
...
async def test_flow_invalid_host_contains_protocol(hass: HomeAssistant) -> None:
    ...
    result2 = await hass.config_entries.flow.async_configure(
        result["flow_id"], {CONF_HOST: "http://validhost", CONF_USERNAME: "u", CONF_PASSWORD: "p"}
    )
    await hass.async_block_till_done()
    assert result2["type"] == data_entry_flow.RESULT_TYPE_FORM
    assert result2["errors"] is not None
    assert result2["errors"].get(CONF_HOST) == "Hostname should not include '://'. Enter just the address."
...
async def test_flow_invalid_host_too_short(hass: HomeAssistant) -> None:
    ...
    result2 = await hass.config_entries.flow.async_configure(
        result["flow_id"], {CONF_HOST: "h", CONF_USERNAME: "u", CONF_PASSWORD: "p"} # Too short
    )
    await hass.async_block_till_done()
    assert result2["type"] == data_entry_flow.RESULT_TYPE_FORM
    assert result2["errors"] is not None
    assert result2["errors"].get(CONF_HOST) == "Hostname is too short or invalid format."
...
async def test_flow_api_connection_error(hass: HomeAssistant) -> None:
    ...
    with patch(
        "custom_components.innotemp.config_flow.InnotempApiClient.async_login",
        side_effect=Exception("Simulated API connection error"), # Simulate any exception during login
    ) as mock_login:
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {
