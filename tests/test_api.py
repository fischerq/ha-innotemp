...
async def test_api_success(mock_client_session):
    ...
    mock_client_session.post.assert_called_with(
        "http://mock_host/inc/groups.read.php", data={"un": "mock_user", "pw": "mock_password"}
    )
    ...
    # 4. SSE signal names
    mock_signal_names_response = await create_mock_response(json_data=["signal1", "signal2"])
    ...
    post_context_login.__aenter__.return_value = mock_login_response # This was used for the first login call
    ...
    assert mock_client_session.post.call_args_list[0][0] == ("http://mock_host/inc/roomconf.read.php",)
    ...
    assert mock_client_session.post.call_args_list[1][0] == ("http://mock_host/inc/value.save.php",)
    assert mock_client_session.post.call_args_list[1][1] == {
        "data": {"room_id": "room1", "param": "param1", "val_new": "new_val", "val_prev": "prev_val"},
    ...
    mock_client_session.get.return_value = sse_get_context_manager # Setup for the GET request

    mock_callback = AsyncMock()
    with patch.object(client, "async_login", new_callable=AsyncMock) as mock_sse_relogin:
        ...
        assert mock_client_session.post.call_args_list[2][0] == ("http://mock_host/inc/live_signal.read.php",)
...
async def test_api_failure(mock_client_session, caplog):
    ...
    mock_invalid_json_response.json = AsyncMock(side_effect=json.JSONDecodeError("Error", "doc", 0))
    ...
    mock_client_session.post.return_value = post_context_invalid_json # Set the context manager mock
    ...
    mock_client_session.post = AsyncMock(return_value=AsyncMock(__aenter__=AsyncMock(return_value=mock_invalid_json_response)))
    ...
    assert any(
        "Failed to decode JSON response" in record.message and f"Response text: '{invalid_json_text}'" in record.message
    ...
    mock_empty_json_response.json = AsyncMock(side_effect=json.JSONDecodeError("Error", "doc", 0))

    mock_client_session.post = AsyncMock(return_value=AsyncMock(__aenter__=AsyncMock(return_value=mock_empty_json_response)))
    ...
    assert any(
        "Failed to decode JSON response" in record.message and f"Response text: '{empty_json_text}'" in record.message
        for record in caplog.records if record.levelname == "ERROR"
    ...
    mock_client_session.post.side_effect = [
        ...
        MagicMock(__aenter__=AsyncMock(return_value=mock_success_after_timeout_response)),
    ]
    ...
    with patch.object(client, "async_login", new_callable=AsyncMock) as mock_relogin:
        ...
        mock_client_session.post.assert_any_call(
            "mock_host/test_endpoint",
            data={"key": "value"},
            cookies={"PHPSESSID": "new_mock_session_id"}, # New session_id after re-login
        )
